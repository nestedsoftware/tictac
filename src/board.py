''' Board class and it's cache '''


from random import choice
from itertools import cycle

from numpy import flipud, fliplr, array, copy, count_nonzero, rot90
...
new_board = array([CELL_EMPTY] * BOARD_SIZE ** 2)


def play_game(x_strategy, o_strategy):
    board = Board()
    player_strategies = cycle([x_strategy, o_strategy])
...
class Board:
    ...
    def get_turn(self):
        non_zero = count_nonzero(self.board)
    ...
    def get_random_valid_move_index(self):
        return choice(self.get_valid_move_indexes())
...
def get_rows_cols_and_diagonals(board_2d):
    ...
    cols_and_antidiagonal = get_rows_and_diagonal(rot90(board_2d))

...
from board import BoardCache, CELL_O, is_empty
...
def get_move_value_pairs(board):
    ...
    move_value_pairs = [(m, get_position_value(board.play_move(m))) for m in valid_move_indexes]
...
def filter_best_move(board, move_value_pairs, randomize):
    ...
    if not randomize:
        return move
    best_move_value_pairs = [mvp for mvp in move_value_pairs if mvp[1] == value]
    chosen_move, _ = choice(best_move_value_pairs)
    return chosen_move
